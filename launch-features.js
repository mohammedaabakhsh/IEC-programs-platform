(()=>{
  ...
  function ensureUi(){
    ...
    if(main&&!document.querySelector('#system-info')){
      ...
      <div class="system-grid">
        ...
        <article class="panel form-panel advanced-wide"><h3>سجل الإصدارات</h3><div class="release-list"><div><strong>${VERSION}</strong><p>واجهة جوال محسنة، قائمة جانبية كتطبيق، دعم حواف الآيفون، وتحسين التثبيت والمزامنة.</p></div><div><strong>2.2.0</strong><p>إزالة الصلاحيات المحلية وتحسين موثوقية الربط والتحديث.</p></div><div><strong>2.1.0</strong><p>تعريب قاعدة البيانات وربط Google Sheets.</p></div></div></article>
    ...
    renderHealth();registerPwa();loadStrategicSuite();loadMobileExperience();
  }
  ...
  function loadMobileExperience(){
    if(!document.querySelector('link[href="mobile.css"]')){const l=document.createElement('link');l.rel='stylesheet';l.href='mobile.css';document.head.appendChild(l)}
    if(!document.querySelector('script[src="mobile.js"]')){const s=document.createElement('script');s.src='mobile.js';s.defer=true;document.body.appendChild(s)}
  }
